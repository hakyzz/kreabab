...
class ProductCustomizer extends Component {
    ...
    handleAddToCart = () => {
        const product = { ...this.props.product, id: Math.floor(Math.random() * (1000000 - 100) + 100), price: this.calculatePrice(), bread: this.props.selection.bread, toppings: this.props.selection.toppings, sauces: this.props.selection.sauces };
            ...
            <div className={"product-customizer " }>
                ...
                <IngredientsList type={activeCategory.listType} title={activeCategory.title}>
                    ...
                    {activeCategory.items.map(item => {
                        return <IngredientsListEntry selected={!!this.props.selection[activeCategory.id].includes(item.title)} key={item.title} category={activeCategory.id} handleClick={this.props.handleIngredientItemClick} id={item.id} image={item.image} name={item.name} title={item.title} text={item.text} price={item.price}>{item.title}</IngredientsListEntry>
                    })}
                </IngredientsList>
